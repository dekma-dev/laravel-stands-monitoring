...
<form class="container" action="{{route('monitoring.update', ['id' => $neededEntry[0]['id']])}}" method="post">
  ...
  <h4>Основная информация</h4>
    <div class="form-group">
      ...
      <select id="StateInput" data-bs-toggle="tooltip" data-bs-placement="bottom" name="State" class="form-control" title="Введите True или False, где True - Установлена, а False - Не установлена" required>
        <option value="Установлена"
        @if ($neededEntry[0]["State"] === "Установлена") selected @endif>
        Установлена</option>
        <option value="Не установлена"
        @if ($neededEntry[0]["State"] === "Не установлена") selected @endif>
        Не установлена</option>
      </select>
      
      <label for="ConditionInput">Состояние</label>
      <input data-bs-toggle="tooltip" data-bs-placement="bottom" title="Введите состояние работоспособности метки в %" type="text" name="Condition" class="form-control" id="ConditionInput" placeholder="Введите состояние работоспособности метки в %" value="{{$neededEntry[0]['Condition']}}"required>
    </div>
    <hr/>
