...
<form class="container" action="{{route('monitoring.update', ['id' => $neededEntry[0]['id']])}}" method="post">
  ...
  <div class="form-group">
    ...
    <input type="text" title="Состояние работоспособности метки в %" name="Condition" class="form-control" id="ConditionInput" value="{{$neededEntry[0]['Condition']}}" placeholder="Введите состояние работоспособности метки в %" required>
  </div>
  <hr/>
  ...
  <div class="form-group">
    ...
    <input type="text" name="Authenticity" class="form-control" id="authInput" value="{{$neededEntry[0]['Authenticity']}}" placeholder="Является ли метка официально зарегистрированной?" required>
  </div>
